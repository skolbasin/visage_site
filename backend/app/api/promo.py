...
def update_promocode(
    ...
    if promo_data.code:
        ...
        existing = db.query(PromoCode).filter(PromoCode.code == promo_data.code, PromoCode.id != promo_id).first()
        if existing:
            raise HTTPException(status_code=400, detail="Promo code already exists")
...
def check_promocode(
    ...
    promo = db.query(PromoCode).filter(PromoCode.code == code, PromoCode.is_used == False).first()
    if not promo:
...
def generate_unique_code(db: Session, length: int = 10) -> str:
    ...
    while True:
        code = "PROMO" + ''.join(random.choices(string.ascii_uppercase + string.digits, k=length))
